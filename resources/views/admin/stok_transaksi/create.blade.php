                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.stok-transaksi.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="barang_id" name="barang_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                <option value="">-- Pilih Barang --</option>
                                @foreach ($barangs as $barang)
                                    <option value="{{ $barang->id }}" {{ old('barang_id') == $barang->id ? 'selected' : '' }} data-stok="{{ $barang->stok_sekarang }}">{{ $barang->nama_barang }}</option>
                                @endforeach
                            </select>
                            @error('barang_id')
